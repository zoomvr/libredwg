  ...
  SINCE (R_2004)
    {
      ...
      // get together all the section sizes, and set the addresses
      {
        ...
        if (!dwg->header.section_info)
          dwg->header.section_info = calloc (26, sizeof (Dwg_Section_Info));
        ...
        for (sec_id = 0; sec_id <= SECTION_EMPTY; sec_id++, i++)
          {
                ...
                do
                  {
                    ...
                    dwg->header.section_info[si].fixedtype = sec_id;
                    dwg->header.section_info[si].type = sec_id;
                    dwg->header.section_info[si].unknown = 1;
                    if (name)
                      strcpy (dwg->header.section_info[si].name, name);
                    dwg->header.section_info[si].size = ssize;
                    dwg->header.section_info[si].max_decomp_size = max_decomp_size;
#ifdef HAVE_COMPRESS_R2004_SECTION
                    dwg->header.section_info[si].compressed = 1 + section_compressed (dwg, sec_id);
#else
                    dwg->header.section_info[si].compressed = 1;
#endif
                    dwg->header.section_info[si].encrypted = 0;
                    dwg->header.section_info[si].num_sections++;
                    dwg->header.section[si].compression_type = dwg->header.section_info[si].compressed;
        ...
        if (dwg->header.num_sections > 28)
          {
            ...
            dwg->header.section_info = realloc (dwg->header.section_info, _obj->numsections * sizeof (Dwg_Section_Info));
          }
            ...
            for (j = 0; j < dwg->header.num_sections; j++)
              {
                ...
                if (info->fixedtype == j)
                  {
                    Dwg_Section *sec = &dwg->header.section[j];
                    LOG_TRACE ("\n=== Write %s @%lu (%u) ===\n", info->name, dat->byte, sec->size);
